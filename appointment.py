...
"""
 ...
 in order to serve a bot which manages appointments.
...
def get_duration(appointment_type):
    appointment_duration_map = {'gre': 30, 'admission': 60, 'toefl': 30}
...
def build_options(slot, appointment_type, date, booking_map):
    ...
    if slot == 'AppointmentType':
        return [
            {'text': 'gre (30 min)', 'value': 'gre'},
            {'text': 'admission (60 min)', 'value': 'admission'},
            {'text': 'toefl (30 min)', 'value': 'toefl'}
        ]
    elif slot == 'Date':
